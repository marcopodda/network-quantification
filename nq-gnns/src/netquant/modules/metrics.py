...
def accuracy(
    ...
    """Calculate Accuracy by classes and draw confusion matrix.
    ...
    for target_i in np.unique(targets):
        ...
        if verbose:
            print(f"class {target_i}: {len(curr_targets)}: accuracy={acc_i:.5f}  ")

    ConfusionMatrixDisplay.from_predictions(targets, preds, labels=np.unique(targets))
